...
# 今天产生的爬虫文件
file_path = abspath(__file__).rsplit('/', 4)[0] + '/files'
file_list = ['%s/%s' % (file_path, file) for file in os.listdir(file_path) if file.endswith('.csv')]

for tem in file_list:
    print(tem)

# 当前文件路径
file_path = abspath(__file__).rsplit('/', 3)[0] + '/file_log/un_tem_file/spider_files/spider_%s.csv' % dt
